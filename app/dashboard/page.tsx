...
export default function Dashboard() {
  ...
  if (!roadmap && !loading) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", textAlign: "center", padding: "0 20px" }} className="fade-up">
       <div style={{ width: "80px", height: "80px", background: "rgba(var(--invert-rgb),0.02)", borderRadius: "24px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "32px", marginBottom: "24px" }}>🛰️</div>
       <h1 style={{ fontSize: "2rem", fontWeight: "900", letterSpacing: "-0.04em" }}>Ready to launch?</h1>
       <p style={{ color: "var(--text-3)", fontSize: "14px", maxWidth: "340px", margin: "12px auto 32px", lineHeight: "1.6" }}>You haven't generated a study plan yet. Describe your goal or upload a syllabus to begin your journey.</p>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "32px" }} className="fade-up dashboard-container">
        ...
        <div>
          ...
          {roadmap && (
             <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "8px", marginTop: "8px" }}>
                ...
                <span style={{ fontSize: "13px", color: "var(--text-3)", fontWeight: "500" }}>{roadmap.goal}</span>
             </div>
          )}
        </div>
        <Link href="/onboarding" style={{ color: "var(--purple)", fontSize: "13px", fontWeight: "800", textDecoration: "none", borderBottom: "2px solid rgba(124,58,237,0.2)", paddingBottom: "2px" }}>New Plan</Link>
      </div>
      ...
      {lastTopic && (
        <div className="card resume-card" style={{ padding: "32px", background: "linear-gradient(135deg, rgba(124,58,237,0.12), rgba(79,70,229,0.06))", border: "1px solid rgba(124,58,237,0.25)", position: "relative", overflow: "hidden" }}>
           ...
           <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "24px" }} className="mobile-stack">
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: "10px", fontWeight: "900", color: "var(--purple-light)", textTransform: "uppercase", letterSpacing: "0.2em", marginBottom: "12px" }}>Resume Session</p>
                <h2 style={{ fontSize: "22px", fontWeight: "900", color: "var(--text-1)", marginBottom: "8px", lineHeight: "1.2" }}>{lastTopic.title}</h2>
                <p style={{ fontSize: "14px", color: "var(--text-3)", fontWeight: "500" }}>Pick up exactly where you left off.</p>
              </div>
      ...
      {/* Stats Section */}
      <div>
        <p style={{ fontSize: "11px", fontWeight: "900", color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.15em", marginBottom: "16px" }}>Performance</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: "16px" }} className="stagger">
          {[
            { label: "Lessons", value: stats.total, color: "var(--text-1)" },
            { label: "Mastered", value: stats.completed, color: "var(--green-light)" },
            { label: "Quizzes", value: stats.quizzes, color: "var(--purple-light)" },
            { label: "Progress", value: `${pct}%`, color: "var(--purple-light)" },
          ].map(s => (
            <div key={s.label} className="card card-interactive" style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
              <span style={{ fontSize: "10px", fontWeight: "800", color: "var(--text-3)", textTransform: "uppercase" }}>{s.label}</span>
