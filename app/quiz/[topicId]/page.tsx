...
export default function QuizPage() {
  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "50vh", flexDirection: "column", gap: "14px" }}>
      ...
      <p style={{ color: "#52525e", fontSize: "13px" }}>Generating quiz...</p>
    </div>
  )
    ...
    return (
      <div className="fade-up" style={{ maxWidth: "440px", margin: "60px auto", textAlign: "center" }}>
        <div className="card" style={{ padding: "40px 36px" }}>
          ...
          <p style={{ color: "#52525e", fontSize: "14px", marginBottom: "24px" }}>
            You scored <strong style={{ color: pct >= 70 ? "#4ade80" : "#fbbf24" }}>{score}/{qs.length}</strong> ({pct}%)
          </p>
          {wrong.length > 0 && (
            <div style={{ textAlign: "left", marginBottom: "24px", padding: "14px", background: "rgba(248,113,113,0.05)", border: "1px solid rgba(248,113,113,0.12)", borderRadius: "8px" }}>
              ...
              {wrong.map((w, i) => <p key={i} style={{ fontSize: "12px", color: "#8b8b99", marginBottom: "4px" }}>• {w}</p>)}
            </div>
          )}
  ...
  if (!qs.length) return <p style={{ color: "#52525e" }}>No questions generated.</p>
    ...
    <div className="fade-up" style={{ maxWidth: "600px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          {qs.map((_, i) => (
            <div key={i} style={{ width: "8px", height: "8px", borderRadius: "50%", background: i < idx ? "#7c3aed" : i === idx ? "#f0f0f4" : "rgba(255,255,255,0.1)", transition: "background 0.2s" }} />
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{ padding: "28px" }}>
        <p style={{ fontSize: "11px", fontWeight: "600", color: "#7c3aed", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: "12px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {q.options.map((opt, i) => {
            let bg = "rgba(255,255,255,0.02)", border = "var(--border)", color = "#b0b0c0"
            if (checked) {
              if (i === q.correctAnswer) { bg = "rgba(22,163,74,0.08)"; border = "rgba(22,163,74,0.25)"; color = "#4ade80" }
        ...
        {checked && (
          <div style={{
            ...
            <p style={{ fontSize: "11px", fontWeight: "600", color: "#a78bfa", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "6px" }}>Explanation</p>
            <p style={{ fontSize: "13px", color: "#8b8b99", lineHeight: "1.6" }}>{q.explanation}</p>
          </div>
        )}
